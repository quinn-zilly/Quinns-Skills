#!/usr/bin/env node
// UserPromptSubmit hook: re-assert the plain-English rules so the style does not drift.
...
process.stdin.on("end", () => {
  ...
  if (/\b(stop speaking plainly|stop plain ?english|stop ste|normal english)\b/i.test(prompt)) {
    process.exit(0);
  }

  const context =
    "PLAIN ENGLISH ACTIVE: reply PLAIN. One idea per sentence, 20 words max, active " +
